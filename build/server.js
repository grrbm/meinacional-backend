...
exports.readPdfFile = exports.getPaymentCode = exports.getMeiHistory = void 0;
...
const getMeiHistory = async (cnpj) => {
    ...
    try {
            ...
            setTimeout(async () => {
                ...
                if (browser && page) {
                    try {
                        ...
                        console.log("Could NOT take screenshot. Most likely browser has already been closed.");
                    }
                }
                resolve({
                    success: false,
        ...
        const meiHistoryPromise = new Promise(async (resolve, reject) => {
            ...
            const emitirGuiaPagamento = await page.waitForSelector(emitirGuiaPagamentoSelector);
            ...
            let exampleArray = await page.$$(`li[data-original-index]:not([class^='disabled']) > a > span.text`);
            ...
            for (let i = 0; i < amountAvailableYears; i++) {
                ...
                exampleArray = await page.$$(`li[data-original-index]:not([class^='disabled']) > a > span.text`);
                exampleArray.reverse();
                exampleArray.pop();
                ...
                const tableOrError = await page.waitForSelector([`table.table`, `div.toast-message`].join(","));
                const foundError = await page.evaluate((tableOrError, toastClass) => tableOrError.classList.contains(toastClass), tableOrError, "toast-message");
                if (foundError) {
                    //Gotta get the puppeteer selector again because the page is auto-refreshed
                    //and it loses context.
                    exampleArray = await page.$$(`li[data-original-index]:not([class^='disabled']) > a > span.text`);
                    exampleArray.reverse();
                    exampleArray.pop();
                    const toastMessage = await page.evaluate((el) => el?.innerText, tableOrError);
                ...
                const tableInnerHtml = await page.$eval("table.table", (element) => element.innerHTML);
                const rows = await page.evaluate(() => {
                    const months = Array.from(document.querySelectorAll("tr.pa > td:not(.multa):nth-child(2)"), (e) => e.innerHTML.trim());
                    const apurados = Array.from(document.querySelectorAll("tr.pa > td:not(.multa):nth-child(3)"), (e) => e.innerHTML.trim());
                    const situations = Array.from(document.querySelectorAll("tr.pa > td:not(.vencimento):nth-child(5)"), (e) => e.innerHTML.trim());
...
exports.getMeiHistory = getMeiHistory;
    ...
    try {
        ...
        const paymentCodePromise = new Promise(async (resolve, reject) => {
            ...
            const found = await page.evaluate((year) => {
                const elements = Array.from(document.querySelectorAll(`li[data-original-index]:not([class^='disabled']) > a > span.text`));
            ...
            if (found.length > 0) {
                ...
                const foundCheckbox = await refreshedPage.evaluate((monthYear) => {
                    const checkboxes = Array.from(document.querySelectorAll("tr.pa > td:nth-child(1) > input"));
                    const months = Array.from(document.querySelectorAll("tr.pa > td:nth-child(2)"));
                ...
                if (foundCheckbox.length > 0) {
                    ...
                    //
                    const emitirButtonSelector = await refreshedPage.waitForSelector("#btnEmitirDas");
                    await emitirButtonSelector.click();
                    const printDASbutton = await refreshedPage.waitForXPath('//a[@href="' +
                    ...
                    refreshedPage.on("response", async (response) => {
                        ...
                        console.log("this is the headers: " + JSON.stringify(response.headers()));
                        if (disposition && disposition.indexOf("attachment") !== -1) {
                            ...
                            const { success, res1, res2, res3, res4 } = await (0, exports.readPdfFile)(full);
                            if (success) {
...
exports.getPaymentCode = getPaymentCode;
const readPdfFile = async (filename = "/Users/guilhermereis/Downloads/DAS-PGMEI-38294699000112-AC2022.pdf") => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const onFinish = (fullstring) => {
                ...
                const [not, res1] = fullstring.match(new RegExp("Página:(.*)AUTENTICAÇÃO MECÂNICA"));
                console.log("res1: " + JSON.stringify(res1, null, 2));
                //console.log("full string after: " + fullstring);
                const [not2, res2] = fullstring.match(new RegExp("Valor Total do Documento(.*)CNPJ Razão Social"));
                console.log("res2: " + JSON.stringify(res2, null, 2));
                const [not3, res3] = fullstring.match(new RegExp("Razão Social(.*)Código Principal"));
                console.log("res3: " + JSON.stringify(res3, null, 2));
                const [not4, res4] = fullstring.match(new RegExp("Pagar este documento até(.*)Observações CPF"));
...
module.exports = {
    getMeiHistory: exports.getMeiHistory,
    getPaymentCode: exports.getPaymentCode,
    readPdfFile: exports.readPdfFile,
};
